...
<main class="max-w-7xl mx-auto px-6 py-6">
  ...
  <div class="grid md:grid-cols-2 gap-8 mb-10">
    <!-- LEFT: Image - Fixed consistent aspect ratio -->
    <div class="sticky top-24 self-start">
      <div class="rounded-2xl overflow-hidden bg-white border border-[rgba(27,94,32,0.08)] product-image-wrapper aspect-square">
        <img src="{{ asset($product->image ?: 'images/lettuce/hero-farm.png') }}" loading="eager" onerror="this.onerror=null;this.src='{{ asset('images/lettuce/hero-farm.png') }}';" class="w-full h-full object-cover object-center" alt="{{ $product->name }}">
      </div>
    </div>
    ...
    <div>
      ...
      <!-- Action Buttons - Matched Height & Width, Responsive -->
      <div class="mb-5 space-y-3">
        @if ($isLoggedIn && $available > 0)
          <div class="flex items-center gap-3">
            <div class="flex items-center border border-[rgba(27,94,32,0.12)] rounded-xl overflow-hidden h-12">
              <button type="button" onclick="let n=this.nextElementSibling;n.value=Math.max(1,parseInt(n.value)-1)" class="px-4 h-12 font-black hover:bg-[#e8f5e9] transition-colors">−</button>
              <input type="number" id="productQty" value="1" min="1" max="{{ $available }}" class="w-16 text-center font-bold text-sm border-x border-[rgba(27,94,32,0.12)] h-12 outline-none" readonly onkeydown="return false" />
              <button type="button" onclick="let n=this.previousElementSibling;n.value=Math.min({{ $available }},parseInt(n.value)+1)" class="px-4 h-12 font-black hover:bg-[#e8f5e9] transition-colors">+</button>
            </div>
            <span class="text-xs text-[#5a7a5c] font-bold">{{ $available }} available</span>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <button type="button" onclick="addToCart({{ $product->id }}, parseInt(document.getElementById('productQty').value))" class="h-12 rounded-xl bg-[#17611f] text-white font-black text-sm flex items-center justify-center gap-2 hover:bg-[#14521a] active:scale-[0.98] transition-all shadow-sm">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l2.4 11.4a2 2 0 0 0 2 1.6h7.9a2 2 0 0 0 1.9-1.4L21 7H6"/><circle cx="10" cy="20" r="1"/><circle cx="18" cy="20" r="1"/></svg>
              Add to Cart
            </button>
            <form method="POST" action="{{ route('cart.add') }}" class="h-12">
              @csrf
              <input type="hidden" name="id" value="{{ $product->id }}">
              <input type="hidden" name="qty" value="1" id="buyNowQty">
              <input type="hidden" name="redirect" value="checkout">
              <button type="submit" onclick="document.getElementById('buyNowQty').value = document.getElementById('productQty')?.value || 1" class="w-full h-12 rounded-xl bg-orange-500 text-white font-black text-sm flex items-center justify-center gap-2 hover:bg-orange-600 active:scale-[0.98] transition-all shadow-sm">
        ...
        @elseif ($available < 1)
          <button disabled class="w-full h-12 rounded-xl bg-gray-200 text-gray-500 font-black text-sm cursor-not-allowed">Temporarily Unavailable</button>
        @else
          <div class="flex items-center gap-3">
            <div class="flex items-center border border-[rgba(27,94,32,0.12)] rounded-xl overflow-hidden h-12">
              <button type="button" onclick="let n=this.nextElementSibling;n.value=Math.max(1,parseInt(n.value)-1)" class="px-4 h-12 font-black hover:bg-[#e8f5e9]">−</button>
              <input type="number" id="productQty" value="1" min="1" max="999" class="w-16 text-center font-bold text-sm border-x border-[rgba(27,94,32,0.12)] h-12 outline-none" readonly onkeydown="return false" />
              <button type="button" onclick="let n=this.previousElementSibling;n.value=parseInt(n.value)+1" class="px-4 h-12 font-black hover:bg-[#e8f5e9]">+</button>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <button type="button" onclick="addToCart({{ $product->id }}, parseInt(document.getElementById('productQty').value || 1))" class="h-12 rounded-xl bg-[#17611f] text-white font-black text-sm flex items-center justify-center gap-2 hover:bg-[#14521a] active:scale-[0.98] transition-all shadow-sm">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l2.4 11.4a2 2 0 0 0 2 1.6h7.9a2 2 0 0 0 1.9-1.4L21 7H6"/><circle cx="10" cy="20" r="1"/><circle cx="18" cy="20" r="1"/></svg>
              Add to Cart
            </button>
            <a href="{{ route('login') }}" class="h-12 rounded-xl bg-orange-500 text-white font-black text-sm flex items-center justify-center gap-2 hover:bg-orange-600 active:scale-[0.98] transition-all shadow-sm">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
              Buy Now
            </a>
          </div>
