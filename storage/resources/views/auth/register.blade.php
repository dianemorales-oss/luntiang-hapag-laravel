    ...
    <form class="space-y-5" method="POST" action="{{ route('register.submit') }}">
      ...
      <div>
        ...
        <input type="email" id="email" name="email" placeholder="your@email.com" required value="{{ old('email') }}"
               class="w-full rounded-xl border border-[rgba(27,94,32,0.12)] px-4 py-3 text-sm text-[#1a2e1c] placeholder-[#9e9e9e] focus:outline-none focus:ring-2 focus:ring-[#52b788]/40 focus:border-[#52b788] transition-colors" />
        <p id="emailError" class="hidden mt-2 text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2"></p>
        <p class="mt-1.5 text-[11px] text-[#9e9e9e]">Must be valid format like name@example.com • Must contain letters/numbers before @</p>
      </div>
...
<script>
  ...
  function validateEmailRealtime(){
    ...
    if(!email){ emailError.classList.add('hidden'); emailInput.classList.remove('border-red-300'); return true; }
    // Basic format
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
      emailError.textContent = 'Invalid email format. Use name@example.com';
    ...
    if(!/[a-zA-Z0-9]/.test(local)){
      emailError.textContent = 'Email must contain letters or numbers before @. Example: john@example.com, not !!!@example.com';
      emailError.classList.remove('hidden');
      emailInput.classList.add('border-red-300');
      return false;
    }
    if(/^[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?`~]+$/.test(local)){
      emailError.textContent = 'Email cannot contain only special characters before @. Please include letters or numbers.';
      emailError.classList.remove('hidden');
      emailInput.classList.add('border-red-300');
      return false;
    }
    if(/^\.|\.$|\.\./.test(local) || /@.*\.\./.test(email) || /^[\.\-]|[\.\-]$/.test(email.split('@')[1]||'')){
      emailError.textContent = 'Email cannot start/end with dot or have consecutive dots.';
      emailError.classList.remove('hidden');
      emailInput.classList.add('border-red-300');
      return false;
    }
