...
class HomeController extends Controller
{
    ...
    {
        try {
            $featured = Product::where('is_active', 1)->orderByDesc('is_best_seller')->orderByDesc('plants_available')->limit(8)->get();
        } catch (\Exception $e) {
            // fallback to catalog
            $catalog = \App\Helpers\LettuceCatalog::get();
            $featured = collect(array_slice($catalog, 0, 8))->map(function($p, $i){
