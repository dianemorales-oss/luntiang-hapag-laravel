    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      @foreach($featured as $p)
        ...
        <article class="product-card bg-white rounded-xl overflow-hidden border border-[rgba(27,94,32,0.08)] flex flex-col">
          <a href="{{ route('products.show', $pslug) }}" class="block relative overflow-hidden product-image-wrapper aspect-square bg-[#f4faf5]">
            <img src="{{ asset($pimg) }}" loading="lazy" onerror="this.onerror=null;this.src='{{ asset('images/lettuce/hero-farm.png') }}';" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="{{ $pname }}">
            @if($p->is_new ?? false)<b class="absolute left-2 top-2 rounded-md bg-[#17611f] px-2 py-1 text-[10px] font-black text-white shadow-sm">New</b>@elseif($pbest)<b class="absolute left-2 top-2 rounded-md bg-[#f9a825] px-2 py-1 text-[10px] font-black text-white shadow-sm">🏆 Best</b>@endif
          </a>
          <div class="p-3">
