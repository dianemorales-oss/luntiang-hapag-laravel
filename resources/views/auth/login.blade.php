  ...
  <div class="w-full max-w-md bg-white rounded-2xl border border-[rgba(27,94,32,0.08)] shadow-sm p-9">
    ...
    @if (session('success') || session('error'))
      ...
      <div id="alertMessage" class="mb-6 flex items-start gap-3 rounded-2xl border px-5 py-4 shadow-sm transition-all duration-500
        ...
        <div class="mt-0.5">
          ...
          @else
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-[#17611f]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <form class="space-y-5" method="POST" action="{{ route('login.submit') }}">
      ...
      <div>
        ...
        <input type="text" id="login" name="login" placeholder="your@email.com or 09123456789" required value="{{ old('login') }}"
